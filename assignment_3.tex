...
\begin{homeworkProblem}
    ...
    \begin{quoting}
        ...
        DES and RSA take two fundamentally different approaches when it comes
        to trying to encrypt a given message. Both have their advantages and
        ...
        keys used for encryption and decryption are different. To do this a key
        pair must be generated, a key pair is required for each party wishing
        to communicate and contains a public and a private key. The public key
        can be shared with the world while the private key must remain a
        secret. To send encrypt a message to someone you must encrypt the
        message you wish to send with the persons public key. The encrypted
        message can only be decrypted by it's corresponding private key. The
        encryption algorithm uses modular exponentiation of messages/keys in
        ...
        an algorithm is. Currently it is considered impossible to crack RSA
        encryption using a keysize of 2048 bit using the computing power
        available today. However it's relatively trivial to crack a 56 bit DES
        ...
        is considered to be reasonably secure by todays standard. RSA has the
        obvious advantage over DES when it comes to the cryptographic security
        of the encrypted messages. \\
        
        As technology and techniques for breaking keys improves current
        keysizes will eventually become insecure. Considering the abundance of
        cheap hard drive space today it's not expensive for an attacker to
        simply save your encrypted messages today and either spend a long time
        cracking the key (3DES) or wait for technology to reach a stage where
        that keysize is no longer secure (RSA). It's easy to conclude that the
        messages we encrypt today will not be secure forever. One advantage RSA
        has over DES/3DES is how easy it is to increase the key size, it's
        simply a matter of increasing the size of the prime numbers you select.
        The larger the keysize you select the longer you can expect your
        ...
        RSA that key is only known by the two parties communicating, and
        because of this attackers cannot generate cipher texts with the key. \\

        A significant disadvantage of DES is unknown nature of the S-Box's
        provided by the NSA. Because no papers have ever been released on how
        they were designed, there is not way to be certain that the S-Box's do
        not have a purposely introduce vulnerability. Thankfully with RSA we
        are able to mathematically see that the encryption algorithm is hard to
        break. \\
        ...
        over the network. We must ensure the key remains a secret, if it is
        ...
        By combining both RSA and DES we are able to get the best of both
        worlds. By transmitting the key with RSA we know the message will only
        be visible to the public key we encrypt it for. On the other side the
        other party knows that we sent it as we are able to digitally sign our
        message. \textit{Dicussion on the sharing/verification of public keys
...
\begin{homeworkProblem}
    ...
    \begin{quoting}
        We know that the modular multiaplicative inverse exists if $a$ and $b$
        are coprime.
        \begin{equation*}ax\equiv1 \pmod m\end{equation*}

        We know that $q$ is going to be coprime with any number besides
        itself given it's a prime number. We also know that k is selected
        randomly such that $0 < k < q$. You can see that $k$ is always less
        than $q$. As such we know that k and m are always coprime and thus
        a modular multiplicative inverse exists.

        When we receive a message and wish to verify it's authenticity we have:
        \begin{equation*}(m', r', s')\end{equation*}
        The $'$ denotes that we haven't verified it to be true. Where lack of
        represents the original values.
    \end{quoting}

    \begin{align*}
        w &= (s')^{-1} \mod q &= s^{-1} \mod q\\
        u_{1} &= [H(m')w] \mod q &= [H(m)w] \mod q\\
        u_{2} &= r'w \mod q &= rw \mod q\\
    \end{align*}
    ...
    \end{quoting}
    \begin{align*}
        g^{q} \mod p &= (h^{\frac{(p-1)}{q}} \mod p) \\
        \shortintertext{given that p is prime}
                     &= h^{(p-1)} \mod p \\
                     &= 1
    \end{align*}

    \begin{quoting}
        Proof that the component added to the end of the hash can be removed
        during verification.
    \end{quoting}
    \begin{align*}
        m' &= m + kq \\
        g^{m'} \mod p &= g^{m + kq} \mod p \\
                      &= (g^{m}g^{kq}) \mod p \\
                      &= (g^{m}(g^{q} \mod p)^{k}) \mod p \\
        \shortintertext{Given the $g^{q} \mod p = 1$}
                      &= g^{m} \mod p
    \end{align*}
    \begin{quoting}
        To verify the signature is valid we must prove that $v = r'$. Where $v$
        is calculated using:
    \end{quoting}
    \begin{align*}
        v &=((g^{u_{1}}y^{u_{2}}) \mod p) \mod q \\
          ...
          &=((g^{H(m)w + xrw}) \mod p) \mod q \\
        \shortintertext{Using the proof above}
          &=((g^{(H(m)w + xr)w}) \mod p) \mod q \\
        \shortintertext{Given:}
        s &= k^{-1}(H(m) + xr) \mod q \\
        \shortintertext{By definition of modular multiplicative inverse:}
        w &= (k(H(m) + xr)^{-1}) \mod q \\
          &\therefore (H(m) + xr)w = k \mod q\\ \\
        v &=((g^{(H(m)w + xr)w}) \mod p) \mod q = r\\
        v &=(g^{k} \mod p) \mod q = r\\
    \end{align*}
\end{homeworkProblem}
